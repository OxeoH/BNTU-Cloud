...
class UserController {
  ...
  //   }
  // }
  public async changeContactPassword(req: Request, res: Response) {
    try {
      const token = req.headers.authorization?.split(" ")[1];
      const userData = verifyTokenMiddleware(token ?? "");
      const { id, newPassword }: ChangeContactPasswordProps = req.body;
      ...
      const candidate = await userService.getUserById(id);
  ...
  public async changeContactProfileInfo(req: Request, res: Response) {
    try {
      const {
        id,
        name,
        ...
        group,
        role,
      }: ChangeContactProfileProps = req.body;
